<template>
  ...
  <div class="login_box">
    ...
    <div class="qrcode">
      <div class="img" ref="qrCodeUrl" v-show="state === 1||state === 3"></div>
      <!-- <img  ref="qrCodeUrl" class="img" :src="imgURL" alt="登录码" v-show="state === 1||state === 3"/> -->
      ...
      <div class="result" v-show="state === 2">
        <p class="u_name">账号：{{userName}}</p>
        <p class="u_name">密码：{{userPasswoed}}</p>
        <p class="u_name">留言：{{userAvatar}}</p>
      </div>
      <div>
...
export default {
  ...
  data () {
    return {
      ...
      userPasswoed:'',
      userName: '', // 扫码登录的用户名
      tokenApi: '/api/token', // 获取口令
      tokenImgApi: '127.0.0.1', // 获取口令对应的登录码
      tokenInfoApi: '/api/info/', // 获取口令信息
      userInfoApi: '/api/getUser', // 获取用户信息
      ceshi:'12354'
    }
  },
  created() { 
  ...
  creatQrCode() {
    var _this = this
    console.log("生成二维码的",_this.imgURL)

    var qrcode = new QRCode(this.$refs.qrCodeUrl, {
    text: _this.imgURL,// 需要转换为二维码的内容
    width: 240,
    height: 240,
    colorDark: '#000000',
    colorLight: '#ffffff',
    ...
    async getToken () {
     ...
     await this.$ajax({
        ...
        this.imgURL = this.tokenImgApi + '?token=' + response.data.data +'&cestcoScanOpenType=scantest'// 拼装获得登录码链接
        console.log("链接是",this.imgURL)
       // console.log(this)
    ...
    getTokenInfo () {
      ...
      this.$ajax({
        method: 'post',
        url: this.tokenInfoApi,
        data: this.qs.stringify({
          token: this.authToken,
        }) 
      }).then((response) => {
        let auth = response.data.data[0]
        console.log("状态码返回" , response.data.data[0].auth_state)
        // token状态为登录成功
        if (auth.auth_state === 1) {
          ...
          this.tip = '登录成功'
          clearInterval(this.timeCount) // 关闭轮询
    ...
    getUserInfo () {
      this.$ajax({
        ...
        this.userAvatar = response.data.data[0].user_avatar
        this.userPasswoed = response.data.data[0].user_password
        console.log(response.data.data)
      }).catch((error) => {
        console.log(error)
      })
